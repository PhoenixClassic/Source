...
public class RegionBBSManager extends BaseBBSManager
{
	...
	private void writeCommunityPages()
	{
		...
		for (int page : _onlinePlayers.keySet())
		{
			...
			StringUtil.append(htmlCode, "<html><body><br><table>" + trOpen + "<td align=left valign=top>Server Restarted: ", String.valueOf(GameServer.dateTimeServerStarted.getTime()), tdClose + trClose + "</table><table width=600>" + trOpen + tdOpen + "XP Rate: x", String.valueOf(Config.RATE_XP), tdClose + colSpacer + tdOpen + "Party XP Rate: x1,5" + tdClose + colSpacer + tdOpen + "Manor rate: x15" + tdClose + trClose + trOpen + tdOpen + "SP Rate: x", String.valueOf(Config.RATE_SP), tdClose + colSpacer + tdOpen + "Party SP Rate: x1,5" + tdClose + colSpacer + tdOpen + "Enchant rate: 66%" + tdClose + trClose + trOpen + tdOpen + "Drop Rate: x40" + tdClose + colSpacer + tdOpen + "Spoil Rate: x", String.valueOf(Config.RATE_DROP_SPOIL), tdClose + colSpacer + tdOpen + "Adena Rate: x25" + tdClose + trClose + "</table><table>" + trOpen + "<td><img src=\"sek.cbui355\" width=600 height=1><br></td>" + trClose + trOpen + tdOpen, String.valueOf(L2World.getInstance().getAllVisibleObjectsCount()), " Object count</td>" + trClose + trOpen + tdOpen, String.valueOf(getOnlineCount("gm")), " Player(s) Online</td>" + trClose + "</table>");
